...
describe("Get Course", () => {
  test("Find Course", async () => {
    ...
    expect(await courseService.getCourse("123")).toEqual(fakeData.courses[0]);
  });
...
describe("Update Course", () => {
  test("Updateing course with wrong data. ID not matching", async () => {
    ...
    const wrongCourse: ICourse = { ...fakeData.courses[0] };

    wrongCourse.id = "234";

    await expect(courseService.updateCourse(wrongCourse)).rejects.toThrowError(
      "Index not found!"
    );
  });
  ...
  test("Updateing course with correct data.", async () => {
    ...
    const newCourse: ICourse = { ...fakeData.courses[0] };
